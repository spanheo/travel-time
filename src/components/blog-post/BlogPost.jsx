...
const BlogPost = () => {
  ...
  const findPost = (pathId) => {
    setPost(
      BlogPostsData.filter((post) => post.title.toLowerCase() === pathId)[0]
    );
  };
  ...
  useEffect(() => {
    ...
    findPost(
      window.location.pathname.toString().substring(6).replaceAll("%20", " ")
    );
  }, [post]);
  return (
    <article className="sm:col-span-5 md:col-span-4 min-h-screen scrollbar-hide p-2 mb-6">
      ...
      <section className="flex flex-col min-h-screen items-center gap-4">
        ...
        />
        <h2 className="text-4xl capitalize text-black dno-underline">
          {post.title}
        </h2>
        <section>
